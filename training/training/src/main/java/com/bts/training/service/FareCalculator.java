...
public class FareCalculator {
    ...
    @Autowired
    private PinkLineFares pinkLineFares;

    @Autowired
    private YellowLineFares yellowLineFares;

    public int calculateFare(List<Station> route) {
        ...
        int additionalFare = calculateAdditionalFares(routeInfo);
        int discount = calculateDiscounts(routeInfo, route);

        int totalFare = mainFare + additionalFare - discount;

        return totalFare;
    }

    private RouteInfo categorizeStations(List<Station> route) {
        RouteInfo routeInfo = new RouteInfo();
        Set<String> seenStations = new HashSet<>();

        for (Station station : route) {
            // Skip if we've already seen this station (handling "CEN1" and "CEN2" equivalence)
            String stationCode = getEquivalentStationCode(station.getCode());
            if (seenStations.contains(stationCode)) {
                continue;
            }
            seenStations.add(stationCode);

            switch (station.getLine().toLowerCase()) {
                case "gold":
                ...
                default:
                    ...
                    if (station.getType().equalsIgnoreCase("normal")) {
                        routeInfo.normalCount++;
                    } else if (station.getType().equalsIgnoreCase("extension")) {
    ...
    private String getEquivalentStationCode(String code) {
        // Add any other equivalent stations here as needed
    ...
    private int calculateAdditionalFares(RouteInfo routeInfo) {
    ...
    private int calculateDiscounts(RouteInfo routeInfo, List<Station> route) {
        int discount = 0;
    ...
    private int getPinkLineFare(List<Station> pinkLineRoute) {
    ...
    private int getYellowLineFare(List<Station> yellowLineRoute) {
    ...
    private boolean containsStation(List<Station> route, String code) {
        String equivalentCode = getEquivalentStationCode(code);
        return route.stream().anyMatch(station -> getEquivalentStationCode(station.getCode()).equals(equivalentCode));
    }

    private int calculateMainFare(int normalCount, int extensionCount) {
    ...
    private int getNormalFare(int normalCount) {
        return NORMAL_FARE_RATES[Math.min(normalCount - 1, NORMAL_FARE_RATES.length - 1)];
    }

    private int calculateBlueLineFare(int stations) {
    ...
    private int calculateArlFare(int stations) {
        return ARL_FARE_RATES[Math.min(stations - 1, ARL_FARE_RATES.length - 1)];
    }

    private int calculatePurpleLineFare(int stations) {
        if (stations == 1) {
            return 16;
        } else if (stations == 2) {
            return 18;
    ...
    private int calculateLadPhraoDiscount(List<Station> route) {
